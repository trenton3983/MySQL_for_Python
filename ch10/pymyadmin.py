    ...
    if qact == "select":
        if calc != "NONE" or distinct != "NONE" or sort != "NONE" or key != "NONE":
            ...
            if sort != "NONE":
                ...
                sorting = ""
            statement = "SELECT %s FROM %s WHERE %s = %s %s" %(selection, tb, cols, vals, sorting)
        else:  
        ...
        if authenticate == 1:
            ...
            if opt.dbact is not None:
                ...
                if opt.sort is not None:
                    ...
                    sort = "NONE"
                    key = "NONE"
                output = qaction(opt.qact, opt.qdbname, opt.qtbname, opt.columns, opt.values, opt.user, opt.password, calc, colkey, distinct, sort, key)
